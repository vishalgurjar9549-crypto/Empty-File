...
import {
  deduplicateAndMerge,
  countDuplicates,
} from "../../utils/roomDeduplicate";
import { ownerService } from "../../services";
import { fetchOwnerRooms, fetchOwnerSummary } from "./owner.slice";
  ...
  async (filters: RoomFilters | undefined, { rejectWithValue }) => {
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        const roomTypesInResponse = [
          ...new Set(response.rooms.map((r) => r.roomType)),
        ];
        console.log(`[Thunk] fetchRooms API response:`, {
          ...
          isFiltered: filters?.roomTypes
            ? roomTypesInResponse.length <= filters.roomTypes.length
            : null,
          meta: response.meta,
...
export const createRoom = createAsyncThunk(
  "rooms/createRoom",
  async (data: CreateRoomInput, { dispatch, rejectWithValue }) => {
    try {
...
export const updateRoom = createAsyncThunk(
  "rooms/updateRoom",
  async (
    { id, data }: { id: string; data: UpdateRoomInput },
    ...
    try {
      ...
      // ✅ 2. Refetch updated data
      await dispatch(fetchOwnerRooms());
      await dispatch(fetchOwnerSummary());
...
export const toggleRoomStatus = createAsyncThunk(
  ...
  async (id: string, { dispatch, rejectWithValue }) => {
    try {
      ...
      const message =
        error.response?.data?.message || "Failed to toggle room status";
      dispatch(
        showToast({
          message,
          type: "error",
  ...
  async (filters: RoomFilters | undefined, { getState, rejectWithValue }) => {
    try {
      ...
      if (process.env.NODE_ENV === "development" && duplicateCount > 0) {
        console.info(
          `[Pagination] ${duplicateCount} duplicate(s) detected, deduplicating`,
          {
      ...
      const message =
        error.response?.data?.message || "Failed to fetch more rooms";
...
const roomsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchRooms.fulfilled, (state, action) => {
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          const roomTypesInStorage = [
            ...new Set(state.rooms.map((r) => r.roomType)),
          ];
          console.log(`[Reducer] fetchRooms.fulfilled:`, {
            oldLength,
            newLength,
            sameReference:
              oldLength === newLength && action.payload.rooms === state.rooms,
    ...
    builder
      .addCase(fetchRoomsWithCursor.pending, (state) => {
        state.loading.fetch = true;
        state.error = null;
      })
      .addCase(fetchRoomsWithCursor.fulfilled, (state, action) => {
        ...
        if (
          ...
          state.rooms.length > 40000
        ) {
          console.warn(
            `[Pagination] Memory: ${state.rooms.length} rooms in state`,
            {
