  ...
  const getPlanBadge = (planName: string) => {
    if (planName === 'FREE') {
      return {
        ...
        icon: Sparkles,
        chip:
          'bg-slate-100/80 dark:bg-slate-700/60 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-slate-600/70',
      };
    }
    ...
    if (planName === 'GOLD') {
      return {
        ...
        icon: Zap,
        chip:
          'bg-[rgba(212,175,55,0.12)] dark:bg-[rgba(212,175,55,0.16)] text-[rgba(170,120,0,1)] dark:text-[rgba(244,211,94,1)] border border-[rgba(212,175,55,0.28)]',
      };
    }
    ...
    if (planName === 'PLATINUM') {
      return {
        ...
        icon: Crown,
        chip:
          'bg-purple-100/80 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200/70 dark:border-purple-700/50',
      };
    }
            ...
            <div className="relative grid grid-cols-1 gap-8 px-5 py-8 sm:px-8 sm:py-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center lg:px-10 lg:py-12">
              <div>
                ...
                <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600 dark:text-slate-400 sm:text-base">
                  ...
                  faster access to verified listings, owner details, and premium
                  discovery tools in <span className="font-semibold text-navy dark:text-white">{cityName}</span>.
                </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex h-full flex-col p-5 sm:p-6 lg:p-7">
                          ...
                          <ul className="space-y-3.5">
                            {Array.isArray(plan.features) &&
                              plan.features.map((feature, i) => (
                                <li
                                  key={i}
                                  className="flex items-start gap-3 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 bg-white/60 dark:bg-slate-800/40 px-3.5 py-3"
                                >
                                  <div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-50 dark:bg-emerald-900/20">
                            ...
                            {planName === 'FREE' && (
                              <>
                                <li className="flex items-start gap-3 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 bg-slate-50/70 dark:bg-slate-800/30 px-3.5 py-3 opacity-75">
                                  <div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-rose-50 dark:bg-rose-900/20">
                        ...
                        <div className="mt-7">
                          <button
                            onClick={() => handleUpgrade(planName, plan.price)}
                            disabled={isCurrent || processingPlan === planName}
                            className={`group inline-flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-3.5 text-sm font-semibold transition-all duration-300 ${styles.button} ${
