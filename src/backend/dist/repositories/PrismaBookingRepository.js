...
const OutboxWriter_1 = require("../services/OutboxWriter");
const OutboxEventTypes_1 = require("../services/OutboxEventTypes");
/**
...
class PrismaBookingRepository {
    ...
    async findByOwnerId(ownerId, page = 1, limit = 20) {
    ...
    async findByTenantId(tenantId, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        const [bookings, total] = await Promise.all([this.prisma.booking.findMany({
                where: {
                    tenantId
    ...
    async createBookingTransactional(bookingData) {
        try {
            const result = await this.prisma.$transaction(async (tx) => {
                ...
                if (!room) {
                    throw new AppErrors_1.NotFoundError('Room', bookingData.roomId);
                }
                if (!room.isActive) {
                    throw new AppErrors_1.BusinessLogicError('Room is not currently available for booking');
                }
                ...
                if (!owner) {
                    throw new AppErrors_1.NotFoundError('Room owner');
                }
                ...
                if (existingBooking) {
                    throw new AppErrors_1.DuplicateBookingError();
                }
                ...
                await (0, OutboxWriter_1.writeOutboxEvent)(tx, {
                    ...
                    eventType: OutboxEventTypes_1.OutboxEventType.BOOKING_CREATED,
            ...
            logger_1.logger.info('Booking created via transaction (with outbox event)', {
     ...
     */
    async updateWithOptimisticLock(id, expectedStatus, data) {
        try {
    ...
    toDomain(prismaBooking) {
        return {
            ...
            status: (typeof prismaBooking.status === 'string' ? prismaBooking.status.toLowerCase() : prismaBooking.status),
