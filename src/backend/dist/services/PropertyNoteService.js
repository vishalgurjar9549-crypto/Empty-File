...
exports.propertyNoteService = exports.PropertyNoteService = exports.NoteAuthorizationError = exports.NoteNotFoundError = exports.NoteValidationError = void 0;
...
class NoteAuthorizationError extends Error {
    constructor(message) {
        super(message);
        this.name = 'NoteAuthorizationError';
    }
}
exports.NoteAuthorizationError = NoteAuthorizationError;
// ============================================================================
// SERVICE CLASS
// ============================================================================
class PropertyNoteService {
     ...
     */
    async canCreateNote(userId, userRole, propertyId) {
     ...
     */
    async canReadNotes(userId, userRole, propertyId) {
        // ADMIN can always read
        if (userRole === client_1.Role.ADMIN) {
            return true;
        }
        // OWNER can read notes for their own properties
        if (userRole === client_1.Role.OWNER) {
        ...
        if (userRole === client_1.Role.TENANT) {
     ...
     * - TENANT: Never
     */
    async canModifyNote(userId, userRole, noteId) {
        ...
        if (userRole === client_1.Role.AGENT && note.authorId === userId) {
            return {
                allowed: true,
                note
            };
     ...
     */
    async createPropertyNote(input) {
        const { propertyId, content, authorId, authorRole } = input;
        logger_1.logger.info(`[PropertyNoteService] Creating note for property ${propertyId} by user ${authorId} (${authorRole})`);
        ...
        logger_1.logger.info(`[PropertyNoteService] Note created: ${note.id}`);
        return {
            id: note.id,
     ...
     */
    async getPropertyNotes(input) {
        const { propertyId, userId, userRole } = input;
        logger_1.logger.info(`[PropertyNoteService] Fetching notes for property ${propertyId} by user ${userId} (${userRole})`);
        ...
        logger_1.logger.info(`[PropertyNoteService] Found ${notes.length} notes for property ${propertyId}`);
     ...
     * - Content must not be empty
     */
    async updatePropertyNote(input) {
        const { noteId, content, userId, userRole } = input;
        logger_1.logger.info(`[PropertyNoteService] Updating note ${noteId} by user ${userId} (${userRole})`);
        // STEP 1: Validate content
        if (!content || content.trim().length === 0) {
            throw new NoteValidationError('Note content cannot be empty');
        }
        // STEP 2: Check authorization and get note
        const { allowed, note } = await this.canModifyNote(userId, userRole, noteId);
        if (!note) {
        ...
        if (note.isDeleted && userRole !== client_1.Role.ADMIN) {
            throw new NoteNotFoundError(`Note not found: ${noteId}`);
        }
        ...
        logger_1.logger.info(`[PropertyNoteService] Note updated: ${noteId}`);
        return {
            id: updatedNote.id,
     ...
     */
    async deletePropertyNote(input) {
        const { noteId, userId, userRole } = input;
        logger_1.logger.info(`[PropertyNoteService] Deleting note ${noteId} by user ${userId} (${userRole})`);
        ...
        if (note.isDeleted) {
            logger_1.logger.info(`[PropertyNoteService] Note ${noteId} already deleted`);
            return {
                success: true,
        ...
        logger_1.logger.info(`[PropertyNoteService] Note soft-deleted: ${noteId}`);
        return {
            success: true,
...
exports.PropertyNoteService = PropertyNoteService;
// Export singleton instance
exports.propertyNoteService = new PropertyNoteService();
//# sourceMappingURL=PropertyNoteService.js.map