...
class IdentityLinkingService {
    ...
    async findOrCreateUserByIdentity(params) {
        const { email, phone, name = "User", role = client_1.Role.TENANT } = params;
        // Normalize inputs
        ...
        if (normalizedEmail) {
            ...
            if (userByEmail) {
                if (!userByEmail.isActive) {
                    throw new AppErrors_1.ForbiddenError("Account has been disabled");
                }
                logger_1.logger.info("Found existing user by email", {
        ...
        // ═════════════════════════════════════════════════════════════════════
        logger_1.logger.info("Creating new user from identity", {
            email: normalizedEmail,
            phone: normalizedPhone
        });
        // Create placeholder password for passwordless users
        const placeholderPassword = await bcryptjs_1.default.hash("", 10);
    ...
    async linkEmailToUser(userId, email) {
        ...
        if (existingUser && existingUser.id !== userId) {
            throw new AppErrors_1.ValidationError("Email already in use by another account");
        }
        ...
        logger_1.logger.info("Linked email to user", {
            userId,
            email: normalizedEmail
        });
        return updatedUser;
    ...
    async linkPhoneToUser(userId, phone) {
        ...
        if (existingUser && existingUser.id !== userId) {
            throw new AppErrors_1.ValidationError("Phone already in use by another account");
        }
        ...
        logger_1.logger.info("Linked phone to user", {
            userId,
            phone: normalizedPhone
        });
        return updatedUser;
    ...
    async linkGoogleIdToUser(userId, googleId) {
        ...
        if (existingUser && existingUser.id !== userId) {
            throw new AppErrors_1.ValidationError("Google account already linked to another user");
        }
     ...
     * ═════════════════════════════════════════════════════════════════════
     */
    async requireEmailVerified(userId) {
...
//# sourceMappingURL=IdentityLinkingService.js.map