...
class AgentService {
     ...
     */
    async getAssignedProperties(agentId) {
        logger_1.logger.info(`[AgentService] Fetching properties for agent: ${agentId}`);
        try {
            ...
            const properties = assignments.map((assignment) => ({
            ...
            logger_1.logger.info(`[AgentService] Found ${properties.length} assigned properties for agent: ${agentId}`);
            return properties;
        }
        catch (error) {
            logger_1.logger.error(`[AgentService] Error fetching properties for agent ${agentId}:`, error);
     ...
     * - Returns empty array if no assignments (no error thrown)
     */
    async getAssignedTenants(agentId) {
        logger_1.logger.info(`[AgentService] Fetching tenants for agent: ${agentId}`);
        try {
            ...
            // Transform to response DTO
            const tenants = assignments.map((assignment) => ({
            ...
            logger_1.logger.info(`[AgentService] Found ${tenants.length} assigned tenants for agent: ${agentId}`);
            return tenants;
        }
        catch (error) {
            logger_1.logger.error(`[AgentService] Error fetching tenants for agent ${agentId}:`, error);
