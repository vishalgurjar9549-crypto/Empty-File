...
exports.notifyAgentPropertyAssigned = notifyAgentPropertyAssigned;
exports.notifyAgentPropertyUnassigned = notifyAgentPropertyUnassigned;
exports.notifyAgentTenantAssigned = notifyAgentTenantAssigned;
exports.notifyAgentTenantUnassigned = notifyAgentTenantUnassigned;
exports.notifyPropertyNoteCreated = notifyPropertyNoteCreated;
 ...
 */
async function notifyAgentPropertyAssigned(assignmentId, adminId) {
    try {
        logger_1.logger.info(`[NotificationIntegration] Preparing AGENT_PROPERTY_ASSIGNED for ${assignmentId}`);
        ...
        if (!assignment) {
            logger_1.logger.warn(`[NotificationIntegration] Assignment not found: ${assignmentId}`);
            return;
        }
        // Fetch admin info
        ...
        // Emit notification
        await NotificationService_1.notificationService.onAgentPropertyAssigned({
        ...
        logger_1.logger.info(`[NotificationIntegration] AGENT_PROPERTY_ASSIGNED sent for ${assignmentId}`);
    }
    catch (error) {
        // Fire-and-forget: log but never throw
        logger_1.logger.error(`[NotificationIntegration] Failed to notify AGENT_PROPERTY_ASSIGNED:`, error);
    }
}
/**
 ...
 */
async function notifyAgentPropertyUnassigned(agentId, propertyId, adminId) {
    try {
        logger_1.logger.info(`[NotificationIntegration] Preparing AGENT_PROPERTY_UNASSIGNED for agent ${agentId}, property ${propertyId}`);
        // Fetch agent
        ...
        if (!agent || !property) {
            logger_1.logger.warn(`[NotificationIntegration] Agent or property not found`);
            return;
        }
        ...
        // Emit notification
        await NotificationService_1.notificationService.onAgentPropertyUnassigned({
        ...
        logger_1.logger.info(`[NotificationIntegration] AGENT_PROPERTY_UNASSIGNED sent`);
    }
    catch (error) {
        logger_1.logger.error(`[NotificationIntegration] Failed to notify AGENT_PROPERTY_UNASSIGNED:`, error);
    }
}
// ============================================================================
 ...
 * @param adminId - The admin who performed the assignment
 */
async function notifyAgentTenantAssigned(assignmentId, adminId) {
    try {
        logger_1.logger.info(`[NotificationIntegration] Preparing AGENT_TENANT_ASSIGNED for ${assignmentId}`);
        // Fetch assignment with all related data
        ...
        // Emit notification
        await NotificationService_1.notificationService.onAgentTenantAssigned({
            assignmentId: assignment.id,
            agentId: assignment.agent.id,
            agentName: assignment.agent.name,
            agentEmail: assignment.agent.email,
        ...
        logger_1.logger.info(`[NotificationIntegration] AGENT_TENANT_ASSIGNED sent for ${assignmentId}`);
    }
    catch (error) {
        logger_1.logger.error(`[NotificationIntegration] Failed to notify AGENT_TENANT_ASSIGNED:`, error);
    }
}
/**
 ...
 * @param adminId - The admin who performed the unassignment
 */
async function notifyAgentTenantUnassigned(agentId, tenantId, adminId) {
    try {
        logger_1.logger.info(`[NotificationIntegration] Preparing AGENT_TENANT_UNASSIGNED for agent ${agentId}, tenant ${tenantId}`);
        // Fetch agent
        const agent = await prisma.user.findUnique({
            where: {
                id: agentId
        ...
        if (!agent || !tenant) {
            logger_1.logger.warn(`[NotificationIntegration] Agent or tenant not found`);
            return;
        }
        // Find the assignment ID (may be deactivated now)
        const assignment = await prisma.agentTenantAssignment.findUnique({
        ...
        // Emit notification
        await NotificationService_1.notificationService.onAgentTenantUnassigned({
        ...
        logger_1.logger.info(`[NotificationIntegration] AGENT_TENANT_UNASSIGNED sent`);
    }
    catch (error) {
        logger_1.logger.error(`[NotificationIntegration] Failed to notify AGENT_TENANT_UNASSIGNED:`, error);
    }
}
// ============================================================================
 ...
 */
async function notifyPropertyNoteCreated(noteId, propertyId, authorId, authorRole) {
    try {
        logger_1.logger.info(`[NotificationIntegration] Preparing PROPERTY_NOTE_CREATED for ${noteId}`);
        // Fetch note
        ...
        if (!note) {
            logger_1.logger.warn(`[NotificationIntegration] Note not found: ${noteId}`);
            return;
        }
        // Fetch property with owner
        const property = await prisma.room.findUnique({
        ...
        if (!property) {
            logger_1.logger.warn(`[NotificationIntegration] Property not found: ${propertyId}`);
            return;
        }
        // Get assigned tenants
        const assignedTenantIds = await NotificationService_1.notificationService.getAssignedTenantsForProperty(propertyId);
        // Create preview (first 100 chars)
        const notePreview = note.content.length > 100 ? note.content.substring(0, 100) + '...' : note.content;
        // Emit notification
        await NotificationService_1.notificationService.onPropertyNoteCreated({
            noteId: note.id,
            propertyId: property.id,
            propertyTitle: property.title,
            propertyCity: property.city,
        ...
        logger_1.logger.info(`[NotificationIntegration] PROPERTY_NOTE_CREATED sent for ${noteId}`);
    }
    catch (error) {
        logger_1.logger.error(`[NotificationIntegration] Failed to notify PROPERTY_NOTE_CREATED:`, error);
    }
}
// ============================================================================
// EXPORTS
// ============================================================================
exports.notificationIntegration = {
