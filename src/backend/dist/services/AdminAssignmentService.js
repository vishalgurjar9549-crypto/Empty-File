...
exports.adminAssignmentService = exports.AdminAssignmentService = exports.NotFoundError = exports.ValidationError = void 0;
...
class AdminAssignmentService {
     ...
     */
    async assignPropertyToAgent(agentId, propertyId, assignedBy, notes) {
        logger_1.logger.info(`[AdminAssignmentService] Assigning property ${propertyId} to agent ${agentId}`);
        ...
        if (agent.role !== client_1.Role.AGENT) {
        ...
        if (existingAssignment) {
            if (existingAssignment.isActive) {
                ...
                logger_1.logger.info(`[AdminAssignmentService] Assignment already exists and is active`);
                assignment = existingAssignment;
            }
            else {
                // Reactivate inactive assignment
                logger_1.logger.info(`[AdminAssignmentService] Reactivating inactive assignment`);
            ...
            logger_1.logger.info(`[AdminAssignmentService] Creating new assignment`);
        ...
        logger_1.logger.info(`[AdminAssignmentService] Property assignment complete: ${assignment.id}`);
        return {
            id: assignment.id,
     ...
     */
    async unassignPropertyFromAgent(agentId, propertyId) {
        logger_1.logger.info(`[AdminAssignmentService] Unassigning property ${propertyId} from agent ${agentId}`);
        ...
        if (!existingAssignment.isActive) {
            ...
            logger_1.logger.info(`[AdminAssignmentService] Assignment already inactive`);
            return {
                success: true,
        ...
        logger_1.logger.info(`[AdminAssignmentService] Property assignment deactivated: ${existingAssignment.id}`);
        return {
            success: true,
     ...
     * - If inactive assignment exists, reactivates it
     */
    async assignTenantToAgent(agentId, tenantId, assignedBy, reason) {
        logger_1.logger.info(`[AdminAssignmentService] Assigning tenant ${tenantId} to agent ${agentId}`);
        // STEP 1: Validate agent exists and has AGENT role
        const agent = await prisma.user.findUnique({
            where: {
                id: agentId
        ...
        if (tenant.role !== client_1.Role.TENANT) {
        ...
        logger_1.logger.info(`[AdminAssignmentService] Tenant assignment complete: ${assignment.id}`);
        return {
            id: assignment.id,
            agentId: assignment.agentId,
     ...
     * - Idempotent - returns success even if already inactive
     */
    async unassignTenantFromAgent(agentId, tenantId) {
        logger_1.logger.info(`[AdminAssignmentService] Unassigning tenant ${tenantId} from agent ${agentId}`);
        // Find existing assignment
        const existingAssignment = await prisma.agentTenantAssignment.findUnique({
            where: {
                agentId_tenantId: {
        ...
        logger_1.logger.info(`[AdminAssignmentService] Tenant assignment deactivated: ${existingAssignment.id}`);
        return {
            success: true,
            message: 'Assignment deactivated successfully'
        };
...
exports.AdminAssignmentService = AdminAssignmentService;
// Export singleton instance
exports.adminAssignmentService = new AdminAssignmentService();
//# sourceMappingURL=AdminAssignmentService.js.map