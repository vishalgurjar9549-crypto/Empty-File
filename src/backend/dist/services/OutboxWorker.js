...
exports.cleanupDeliveredOutboxEvents = cleanupDeliveredOutboxEvents;
exports.getOutboxMetrics = getOutboxMetrics;
exports.getOutboxDetailedStats = getOutboxDetailedStats;
...
const PROCESSING_TIMEOUT_MS = 60000; // 1 minute — if event stuck in PROCESSING
const metrics = {
    eventsProcessed: 0,
    eventsDelivered: 0,
    eventsFailed: 0,
...
function startOutboxWorker() {
    ...
    isRunning = true;
    logger_1.logger.info('Outbox worker: Starting', {
    ...
    pollAndProcess().catch((err) => {
        logger_1.logger.error('Outbox worker: Initial poll failed', {
            error: err.message
        });
    });
    ...
    pollTimer = setInterval(async () => {
        try {
            ...
            logger_1.logger.error('Outbox worker: Poll cycle failed', {
...
async function pollAndProcess() {
  ...
  `);
    ...
    logger_1.logger.info(`Outbox worker: Processing ${events.length} events`, {
...
async function processEvent(event) {
    ...
    try {
        ...
        await prisma.outboxEvent.update({
            ...
            data: {
                status: OutboxEventTypes_1.OutboxEventStatus.DELIVERED,
                processedAt: new Date()
            }
        });
        metrics.eventsDelivered++;
        logger_1.logger.info('Outbox: Event delivered', {
        ...
        if (newRetryCount > (event.maxRetries || OutboxEventTypes_1.RETRY_CONFIG.maxRetries)) {
            ...
            await prisma.outboxEvent.update({
                ...
                data: {
                    status: OutboxEventTypes_1.OutboxEventStatus.DEAD_LETTER,
                    retryCount: newRetryCount,
                    lastError: error.message?.substring(0, 500) || 'Unknown error'
                }
            });
            metrics.eventsDeadLettered++;
            logger_1.logger.error('Outbox: Event dead-lettered (max retries exceeded)', {
            ...
            const nextRetryAt = (0, OutboxEventTypes_1.calculateNextRetryAt)(newRetryCount);
            await prisma.outboxEvent.update({
                where: {
                    id: event.id
                },
                data: {
                    status: OutboxEventTypes_1.OutboxEventStatus.PENDING,
            ...
            logger_1.logger.warn('Outbox: Event failed, scheduled retry', {
...
async function recoverStuckEvents() {
    ...
    const result = await prisma.outboxEvent.updateMany({
        where: {
            status: OutboxEventTypes_1.OutboxEventStatus.PROCESSING,
            updatedAt: {
                lt: cutoff
            }
        },
    ...
    if (result.count > 0) {
        logger_1.logger.warn(`Outbox: Recovered ${result.count} stuck events`, {
 ...
 */
async function routeEvent(eventType, payload, outboxEventId) {
    switch (eventType) {
        case OutboxEventTypes_1.OutboxEventType.BOOKING_CREATED:
            await handleBookingCreated(payload, outboxEventId);
            break;
        case OutboxEventTypes_1.OutboxEventType.BOOKING_STATUS_CHANGED:
            await handleBookingStatusChanged(payload, outboxEventId);
            break;
        case OutboxEventTypes_1.OutboxEventType.BOOKING_CANCELLED:
            await handleBookingCancelled(payload, outboxEventId);
            break;
        default:
            logger_1.logger.warn(`Outbox: Unknown event type: ${eventType}`, {
 ...
 */
async function handleBookingCreated(payload, outboxEventId) {
    ...
    logger_1.logger.info('Outbox handler: BOOKING_CREATED notification sent', {
 ...
 */
async function handleBookingStatusChanged(payload, outboxEventId) {
    if (!payload.tenantId)
        return;
    ...
    logger_1.logger.info('Outbox handler: BOOKING_STATUS_CHANGED notification sent', {
 ...
 */
async function handleBookingCancelled(payload, outboxEventId) {
    await NotificationService_1.notificationService.emit({
        recipientId: payload.ownerId,
    ...
    logger_1.logger.info('Outbox handler: BOOKING_CANCELLED notification sent', {
 ...
 */
async function cleanupDeliveredOutboxEvents(retentionDays = 7) {
    ...
    if (result.count > 0) {
        logger_1.logger.info(`Outbox cleanup: Removed ${result.count} delivered events`, {
 ...
 */
async function getOutboxDetailedStats() {
    try {
        ...
        logger_1.logger.error('Outbox: Failed to get detailed stats', {
            error: error.message
        });
        return {
            ...metrics,
