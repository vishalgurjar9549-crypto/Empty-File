...
class AdminController {
    ...
    async getStats(req, res) {
        try {
            ...
            logger_1.logger.info("Admin stats fetched", {
                stats
            });
            res.json({
                success: true,
            ...
            logger_1.logger.error("Error fetching stats", {
                error: error.message
            });
            res.status(500).json({
                success: false,
    ...
    async getAllUsers(req, res) {
        try {
            ...
            logger_1.logger.info(`Admin: Fetched ${transformedUsers.length} users`);
            res.json({
                success: true,
    ...
    async updateUserStatus(req, res) {
        try {
            ...
            logger_1.logger.info(`Admin: Updated user ${userId} status to ${activeStatus ? "active" : "disabled"}`);
            res.json({
                success: true,
            ...
            logger_1.logger.error("Error updating user status", {
                error: error.message
            });
            res.status(500).json({
                success: false,
    ...
    async getAllProperties(req, res) {
        try {
            ...
            if (status && status !== "all") {
                ...
                where.reviewStatus = statusMap[status] || status;
            }
            if (search) {
                where.OR = [{
                        title: {
            ...
            logger_1.logger.info(`Admin: Fetched ${transformedProperties.length} properties`);
            res.json({
                success: true,
            ...
            logger_1.logger.error("Error fetching properties", {
                error: error.message
            });
            res.status(500).json({
                success: false,
        ...
        try {
            ...
            const updated = await prisma.room.update({
                ...
                data: {
                    reviewStatus: "APPROVED",
                    isActive: true,
                    adminFeedback: client_1.Prisma.JsonNull
                },
                include: {
                    owner: {
                        select: {
            ...
            logger_1.logger.info(`Admin: Approved property ${id}`);
            return res.json({
                success: true,
                data: transformed,
            ...
            logger_1.logger.error("Error approving property", {
                error: error.message
            });
    ...
    async rejectProperty(req, res) {
        try {
            ...
            logger_1.logger.info(`Admin: Rejected property ${id}`);
            res.json({
                success: true,
                data: property,
            ...
            logger_1.logger.error("Error rejecting property", {
                error: error.message
            });
            res.status(500).json({
                success: false,
    ...
    async requestCorrection(req, res) {
        try {
            ...
            logger_1.logger.info(`Admin: Requested correction for property ${id}`);
            res.json({
                success: true,
                data: property,
            ...
            logger_1.logger.error("Error requesting correction", {
                error: error.message
            });
            res.status(500).json({
                success: false,
    ...
    async suspendProperty(req, res) {
        try {
            ...
            logger_1.logger.info(`Admin: Suspended property ${id}`);
            res.json({
                success: true,
                data: property,
            ...
            logger_1.logger.error("Error suspending property", {
                error: error.message
            });
            res.status(500).json({
                success: false,
    ...
    async getActivity(req, res) {
        try {
            ...
            logger_1.logger.error("Error fetching activity", {
                error: error.message
            });
            res.status(500).json({
                success: false,
    ...
    async getTenants(req, res) {
        try {
            ...
            logger_1.logger.info(`Admin: Fetched ${tenants.length} tenants`);
            res.json(tenants);
        }
        catch (error) {
            logger_1.logger.error("Error fetching tenants", {
                error: error.message
            });
            res.status(500).json({
                success: false,
    ...
    async getAgents(req, res) {
        try {
            ...
            logger_1.logger.info(`Admin: Fetched ${agents.length} agents`);
            res.json(agents);
        }
        catch (error) {
            logger_1.logger.error("Error fetching agents", {
                error: error.message
            });
            res.status(500).json({
                success: false,
