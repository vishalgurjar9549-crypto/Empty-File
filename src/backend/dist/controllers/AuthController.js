...
class AuthController {
    ...
    async register(req, res, next) {
        try {
            ...
            const roleMap = {
                ...
                owner: client_1.Role.OWNER,
                agent: client_1.Role.AGENT,
    ...
    async resendVerification(req, res) {
        try {
            ...
            const result = await EmailVerificationService_1.EmailVerificationService.resendVerification(userId);
            if (!result.success) {
                return res.status(400).json({
                    success: false,
                    message: result.message
    ...
    async claimAccount(req, res) {
        try {
            ...
            const result = await this.authService.claimAccount(String(phone).trim(), String(email).trim(), String(password).trim());
            return res.json({
                success: true,
                data: result
            });
    ...
    async verifyEmailOTP(req, res) {
        try {
            ...
            const result = await this.authService.verifyEmailOTP(userId, String(email).trim(), String(otp).trim());
            if (!result.success) {
                return res.status(400).json({
                    success: false,
                    code: result.code,
    ...
    // =======================
    async requestEmailLoginOTP(req, res) {
        try {
            const { email } = req.body;
            if (!email || String(email).trim() === '') {
                return res.status(400).json({
    ...
    // =======================
    async verifyEmailLoginOTP(req, res) {
        try {
            const { email, otp } = req.body;
            if (!email || String(email).trim() === '') {
                return res.status(400).json({
            ...
            const result = await this.authService.verifyEmailLoginOTP(String(email).trim(), String(otp).trim());
            if (!result.success) {
