...
import { ValidationError, DuplicateError, ForbiddenError } from "../errors/AppErrors";
...
export class AuthService {
  ...
  async register(email: string, password: string, name: string, role: Role, phone?: string | null) {
    // Normalize email
  ...
  async claimAccount(phone: string, email: string, password: string) {
    ...
    if (!user) {
      throw new ValidationError("No account found with this phone number. Please sign up first.");
    }

    // Check if this is actually a temp account
    if (!user.email.startsWith("temp_")) {
      throw new ValidationError("This account has already been claimed. Please log in instead.");
    }

    // Check if the new email is already taken
    const existingEmail = await this.userRepository.findByEmail(normalizedEmail);
    if (existingEmail && existingEmail.id !== user.id) {
      throw new DuplicateError("Email is already registered. Please use a different email.");
    }
    ...
    try {
      await EmailVerificationService.generateAndSendVerification(updatedUser.id, updatedUser.email);
    } catch (err: any) {
  ...
  async loginPhone(phone: string) {
    ...
    if (user.email.startsWith("temp_")) {
      ...
      throw new ForbiddenError("This account has not been properly set up. Please claim your account first.");
    }
  ...
  // VERIFY EMAIL OTP
  // =======================
  async verifyEmailOTP(userId: string, email: string, otp: string): Promise<{
    success: boolean;
    code?: string;
    message: string;
    user?: any;
  ...
  // =======================
  async verifyEmailLoginOTP(email: string, otp: string): Promise<{
    success: boolean;
    code?: string;
    message: string;
    user?: any;
